...
const TableGames = () => {
  const swiperRef11 = useRef(null);
  useEffect(() => {
    // Ensure navigation links are ready
    if (swiperRef11.current && swiperRef11.current.swiper) {
      swiperRef11.current.swiper.navigation.update();
    }
  }, []);
  ...
  return (
    <div className="w-dvw max-w-[100%] overflow-hidden rounded-md relative mx-auto bg-gradient-to-b from-[#0e192a] to-[#091222] px-2">
      ...
      <div className="flex flex-row items-center justify-between  py-4">
        ...
        <div className="flex flex-row items-center gap-2 2xl:pr-16">
          <div className="px-2 md:px-4 py-1 bg-[#152436] rounded-md">
            <p className="text-xs md:text-sm text-slate-400">189 Games</p>
          </div>
          {/* Custom Swiper Navigation */}
          <div className="flex flex-row items-center gap-2">
            <div className="table-prev bg-[#152436] w-6 md:w-8 h-6 md:h-8 rounded-md flex items-center justify-center cursor-pointer">
              <IoIosArrowBack className="text-slate-400 text-base md:text-xl" />
            </div>
            <div className="table-next bg-[#152436] w-6 md:w-8 h-6 md:h-8 rounded-md flex items-center justify-center cursor-pointer">
      ...
      <div>
        <Swiper
          ref={swiperRef11} // Attach ref to Swiper
          breakpoints={{
            640: {
